function spawnOne(secretNames) {
  //   ui.showMessage("spawnOne triggered");
  ...
  if (availableNames.length > 0) {
    ...
    park.postMessage({
      type: "blank",
      text: `Renaming guest ${guest.name} (${guest.id}) ${randomName}`,
    });
...
registerPlugin({
  ...
  //TODO: To GUI, have "run now" functionality
  // Allow config for multiple of same name
  ...
  main: function () {
    ...
    const negativeEffects = [
      ...
      "Carol Young", // Decreasing Happiness
      "Corina Massoura", // "It's too crowded here."
      "Donald MacRae", // "I'm lost!"
      "Mia Sheridan", // Increasing Nausea
      "Eilidh Bell", // Vandalizes
      "Katie Rodger", // Wants to Leave
    ];

    let blacklisted = [...negativeEffects];
    ...
    let name = "Secret Character Spawner";
    ui.registerMenuItem(name, function () {
      var window = ui.openWindow({
        ...
        widgets: [
          {
            type: "button",
            text: "Change random guest",
            onClick: () => spawnOne(secretNames),
            x: 5,
            y: 20,
            width: 118,
            height: 12,
