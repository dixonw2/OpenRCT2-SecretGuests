...
function getEligibleCharacters(): SecretCharacter[] {
  ...
  return SECRET_CHARACTERS.filter((character) => {
    if (blacklistedNames.indexOf(character.name) !== -1) {
      return false;
    }

    return getCurrentSecretCount(character.name) < maxPerName;
  });
}
...
function openSecretCharactersWindow() {
  ...
  function getWhitelistNames(): string[] {
    return SECRET_CHARACTERS.map((c) => c.name).filter((c) =>
      getBlacklistNames().every((bc) => bc !== c),
    );
  }
